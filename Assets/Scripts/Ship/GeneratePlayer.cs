...
public class GeneratePlayer : MonoBehaviour {
	...
	void Start(){
		InstantiatePlayer ();
		GameController.instance.blockSize = sizeOffset;
	}
	...
	void InstantiatePlayer(){
		...
		Enter enter = mainEngine.AddComponent<Enter>();
		enter.initialTime = 5;
		enter.slowTime = 3;

		mainEngine.AddComponent<ShipController>();
		...
		gun1.tag = "Player";
		gun1.layer = 8;
		gun1.GetComponent<Gun>().isAttached = true;
		...
		gun2.tag = "Player";
		gun2.layer = 8;
		gun2.GetComponent<Gun>().isAttached = true;
		...
		gun3.tag = "Player";
		gun3.layer = 8;
		gun3.GetComponent<Gun>().isAttached = true;
		...
		GameController.instance.shipSize = (int)Camera.main.orthographicSize;
	}
}
